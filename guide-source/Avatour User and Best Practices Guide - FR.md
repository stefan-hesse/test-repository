...
- 10 spectateurs maximum par réunion.
...
- **Provisionnement automatique via SSO (niveaux Entreprise/Business uniquement) :** Les comptes peuvent être créés automatiquement par le fournisseur d’identité (IdP). Par défaut, les comptes provisionnés via SSO sont ajoutés au **groupe Invité**, sauf si cette règle est remplacée par des **mappages de groupes SAML**. Les administrateurs peuvent toujours inviter des utilisateurs et leur attribuer directement une appartenance à un groupe, même lorsque le SSO est activé.
...
- **Provisionnement SSO :** pour les clients des niveaux « Enterprise » ou « Business » ayant activé le SSO, le fournisseur d’identité (IdP) peut automatiquement provisionner des comptes et attribuer une appartenance à un groupe, ce qui définit le rôle de l’utilisateur sur la plateforme.  
- **Utilisateurs invités par un hôte :** les hôtes peuvent inviter d’autres utilisateurs en tant que collaborateurs « Éditeur » dans des espaces de travail spécifiques. L’attribution du rôle de collaborateur « Éditeur » utilise une licence d’hôte.
...
- Ne peuvent pas créer d’espaces de travail, organiser de réunions ni mettre en ligne de contenu.  
...
- Promouvoir des utilisateurs « Invités » au statut d’animateur.  
- Désactiver des utilisateurs (les comptes « Admin » doivent d’abord être convertis en compte « Animateur » avant d’être supprimés).  
- Transférer des ressources d’un utilisateur « Animateur » à un autre lors de la suppression.
...
3. Contribue à prévenir toute surchauffe potentielle.  
...
- **Environnements plus calmes / plusieurs participants près de la caméra :** 
  Utilisez le haut-parleur à clipser NoxGear. Il n’offre pas la même qualité audio que les haut-parleurs de conférence (par exemple, Jabra Speak), mais il se fixe facilement à votre chemise et capte efficacement les voix à proximité.  
...
**Débit recommandé :** 10 Mbps en upload/download pour un streaming à 360° complet (~5 Mbps). Un débit inférieur (1 à 2 Mbps) ne fonctionne que si vous restez immobile.
...
- **Test en se déplaçant dans les locaux :** Depuis la caméra : Paramètres → Réseau → Test de connexion. Parcourez l’ensemble de l’espace pour vérifier la couverture et la bande passante.
...
- **Capture en direct** : ajustez vos paramètres de capture en direct en fonction de la bande passante disponible, de la sensibilité VR de l’invité ou de la présence ou non des lentilles de protection installées sur votre caméra :
...
- **À propos** : affichez le numéro de série de l'appareil et la version du logiciel
