...
function postHeight() {
  ...
  if (window.parent !== window) {
    window.parent.postMessage({ avatourHeight: h }, '*');
  }
}
...
// ── ANCHOR LINKS ───────────────────────────────────────────────────────
// The parent Webflow page controls scrolling — not the iframe.
// On click we send the target's offset within the iframe to the parent,
// which adds the iframe's own top offset and scrolls to the right place.
document.querySelectorAll('.guide-toc a[href^="#"]').forEach(function(link) {
  link.addEventListener('click', function(e) {
    ...
    if (!target) return;
    var offsetInIframe = target.getBoundingClientRect().top + window.pageYOffset;
    window.parent.postMessage({ avatourScrollTo: offsetInIframe }, '*');
  });
});

// ── TOC STICKY POSITION ────────────────────────────────────────────────
// Parent sends back the iframe's current top offset in the viewport
// whenever the page scrolls. We use this to keep the TOC pinned just
// below the Webflow nav bar.
var tocEl = document.querySelector('.guide-toc');
window.addEventListener('message', function(e) {
  if (!e.data || !tocEl) return;
  if (typeof e.data.avatourIframeTop === 'number') {
    var pad = 20;
    var top = Math.max(pad, e.data.avatourIframeTop + pad);
