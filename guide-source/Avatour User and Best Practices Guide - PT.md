...
Os usuários podem participar da reunião sem se cadastrar para obter uma conta no Avatour. Exceção: Se o anfitrião tiver marcado a opção “Autenticação necessária” nas configurações da reunião (veja também 4.2.1 Configurações para espaços de trabalho e reuniões) — por exemplo, para permitir que apenas funcionários internos participem por meio do Single Sign-On (SSO) —, o convite do calendário indicará que os participantes devem fazer login para se autenticar.
...
- **Provisionamento automático por SSO (apenas nos planos Enterprise/Business):** As contas podem ser criadas automaticamente pelo IdP. Por padrão, as contas provisionadas por SSO são adicionadas ao **grupo de Convidados**, a menos que isso seja substituído por meio de **mapeamentos de grupos SAML**. Os administradores ainda podem convidar usuários e atribuir associação a grupos diretamente, mesmo quando o SSO estiver ativado.
...
- **Provisionamento por SSO:** Para clientes dos planos Enterprise ou Business com SSO ativado, o IdP pode provisionar contas automaticamente e atribuir a filiação a grupos, o que define a função do usuário na plataforma.  
...
- Não podem criar espaços de trabalho, hospedar reuniões nem enviar conteúdo.  
- As contas de convidados provisionadas por SSO **se autenticam por meio do IdP**; não é necessária nenhuma senha gerenciada pelo Avatour.
...
- Podem criar/gerenciar espaços de trabalho, convidar colaboradores para um espaço de trabalho, **organizar reuniões ao vivo** e fazer upload de **Capturas Rápidas**.  
...
- Redefinir senhas quando gerenciadas pelo Avatour (não aplicável quando o SSO estiver habilitado). 
- Elevar usuários convidados à categoria de Anfitrião.  
...
- **Colaborador editor:** Usuários com essa permissão podem:
    ...
    - Gerenciar configurações de reuniões (ativar/desativar gravação, permitir ou remover participantes) 
...
- **Participar de uma reunião no Avatour (colaboração síncrona):**  
  Você pode receber um **convite de calendário** para participar de uma reunião no Avatour. Durante a reunião, os participantes podem realizar uma **visita remota ao local em tempo real** ou analisar ativos em conjunto de forma síncrona.

- **Visitar um Espaço de Trabalho (Colaboração assíncrona):**  
  Você também pode ser convidado como **colaborador de um Espaço de Trabalho** para analisar ativos **de forma assíncrona** (no seu próprio horário).
...
#### 3.1.1 Qualquer dispositivo com “tela plana” e um navegador da Web {#any-flat-screen}
...
2. **Por meio do código da reunião:**  
    - Se o anfitrião compartilhar um **código de reunião de 5 caracteres** separadamente, acesse [https://avatour.live/join](https://avatour.live/join), digite seu **nome** e o **código de reunião** e participe da reunião.  
    - Se a reunião estiver **protegida por senha**, digite a senha fornecida pelo anfitrião. 
 - Se a reunião exigir **autenticação**, você precisará **fazer login com sua conta do Avatour** antes de participar.

> **Dica 1:** Se sua câmera ou microfone não funcionar, talvez estejam sendo usados por outro aplicativo (por exemplo, Microsoft Teams ou Zoom). Feche todos os aplicativos que possam estar usando sua câmera ou microfone e, em seguida, saia e entre novamente na reunião do Avatour.  
...
> O teste pode identificar se seu **firewall corporativo ou rede** está bloqueando o acesso e fornecerá informações para orientar as discussões com sua equipe de TI.  
...
1. **Reuniões (síncronas):** colabore em tempo real com outros participantes, incluindo visitas ao local ao vivo ou a revisão conjunta de recursos gravados.  
...
Clicar em uma opção do menu abre um **painel lateral** no lado esquerdo ou direito da tela, enquanto a **área central** permanece como a principal área de visualização.
...
Aqui está um exemplo de visualização do Espaço de Trabalho:

![Espaço de Trabalho do Avatour com Painel de Ativos, tela em branco e Painel de Reuniões](https://res.cloudinary.com/avatour/image/upload/avatour-screenshot-workspace-assets-blank-meetings_qeumpl.png)  
...
- **Mapa** – Abra ou feche o painel do mapa para ver o movimento da câmera para recursos com trilha de GPS. Clicar em um local leva você diretamente para o ponto exato no vídeo. O mapa é atualizado em tempo real à medida que o vídeo é reproduzido. As anotações também são exibidas no mapa.
...
- **Configurações** – Ajuste as configurações de idioma, áudio e vídeo. Para reuniões com vídeo ao vivo em 360°, use **Mostrar taxa de bits** para monitorar as estatísticas de conectividade.
...
##### Barras de ferramentas de ativos e Live 360° em reuniões

Ao apresentar um ativo em uma reunião, a **Barra de ferramentas de ativos** aparece acima da tela. Aqui estão as ferramentas e os itens de menu disponíveis ao <u>apresentar um ativo em uma reunião</u> – explicados da esquerda para a direita.
...
Quando um vídeo ao vivo em 360° está sendo transmitido em uma reunião, esse menu aparece na parte inferior da tela.
...
##### Barra de ferramentas do ativo (Área de Trabalho)

Ao revisar um recurso em uma área de trabalho, a barra de ferramentas é semelhante, mas otimizada para uso individual:

![Menu do Avatour ao apresentar um recurso fora de uma reunião, por exemplo, ao acessar uma área de trabalho](https://res.cloudinary.com/avatour/image/upload/avatour-screenshot-present-asset-menu-workspace_iri8gc.png) *Menu do Avatour ao apresentar um recurso em um espaço de trabalho*
...
- Controles adicionais:
    - **Intervalos de 10 segundos** – Avançar/retroceder 
 - **Velocidade de reprodução** – Ajustar a velocidade (0,5×–2×) 
 - **Cortar vídeo** – Cortar o início ou o fim do arquivo


## 4. Para usuários do tipo “Host” e “Admin” – Console da Web do Avatour {#for-host-and-admin-users-avatour-web-console}
...
> Vários usuários podem acessar um espaço de trabalho simultaneamente sem a necessidade de uma reunião. Espaços de trabalho públicos e configurações de acesso a reuniões oferecem alternativas de acesso.  
...
- **Mapa** – Exiba as localizações dos ativos com GPS em um mapa, conforme descrito acima para reuniões. 
...
Os usuários administradores têm acesso a este menu para gerenciar centralmente as configurações de toda a plataforma Avatour. Cada configuração pode ser marcada ou desmarcada para se tornar a configuração padrão em toda a plataforma. Cada configuração também pode ser bloqueada, o que significa que a configuração padrão não poderá ser alterada por outros usuários da plataforma. Aqui você também pode fazer personalizações de marketing relacionadas à sua marca (logotipo, cores etc.).

![Console da Web do Avatour - Configurações do Menu Principal](https://res.cloudinary.com/avatour/image/upload/v1781172727/avatour-screenshot-main-menu-settings-1-of-2_fsaatf.jpg) *Seção de configurações*
...
![Console Web do Avatour - Item do menu principal “Análises” (1 de 3)](https://res.cloudinary.com/avatour/image/upload/avatour-screenshot-main-menu-analytics-1-of-3_ds3epe.png) *Visão geral das análises*
...
### 5.1 Introdução
Aqui você encontrará um guia on-line muito completo para dar seus primeiros passos com o Avatour Turnkey Kit: [Guia de Início Rápido – Kit Avatour Turnkey 3.1 (Configuração do Pilot PanoX V2)](https://avatour.com/quickstart-panox-v2)
...
A bateria interna da câmera dura aproximadamente 30 a 45 minutos. Um alerta será exibido quando a bateria estiver acabando. Com uma bateria externa, você pode prolongar o tempo de operação e até mesmo torná-lo ilimitado, já que é possível trocar as baterias durante o uso.

- **Se o seu kit incluir uma bateria Ulanzi:** Encaixe-a entre a base do tripé e a haste extensível e, em seguida, conecte a bateria à câmera via USB-C.  
...
- **Ambientes mais silenciosos / vários participantes próximos à câmera:** 
  Use o alto-falante clipável NoxGear. Ele não tem a mesma alta fidelidade que alto-falantes de conferência (por exemplo, o Jabra Speak), mas é fácil de prender na camisa e capta vozes próximas com eficácia. 
...
- Se o departamento de TI exigir inclusão na lista de permissões, encontre o endereço MAC: Configurações → Sobre → Endereço Wi-Fi.
...
- Solução de problemas:
 ...
 - Verifique se o APN está correto para a sua região ([Guia de configuração de APN](https://avatour.com/support/how-do-i-change-the-apn-on-my-glocalme-hotspot)).
...
Quando vários participantes entram em uma reunião ao vivo do Avatour no mesmo local da câmera 360°, o gerenciamento cuidadoso do **áudio e da largura de banda** é fundamental:  
...
- **Modo Presencial:** Participe da reunião no modo Presencial quando estiver fisicamente presente próximo à câmera 360°, pois esse modo é otimizado para uso no local:
...
Seguir essas orientações garante um tour ao vivo tranquilo e de alta qualidade tanto para participantes presenciais quanto remotos.
...
> Ao hospedar uma reunião do Live Capture com sua câmera 360°, você terá à disposição ferramentas de reunião semelhantes às da experiência na web. Aqui está um link para o artigo da nossa Base de Conhecimento que explica essas ferramentas com mais detalhes: [Ferramentas do aplicativo do operador](https://avatour.com/support/quais-ferramentas-do-aplicativo-Avatour-estão-disponíveis-para-operadores-da-câmera-LabPano-Pilot)

**Galeria** - Encontre aqui todos os seus vídeos e fotos em 360° para enviar para o Avatour Web Console. Você pode enviar e excluir arquivos em lote — toque em “Selecionar” na parte superior da tela. Antes do envio, você pode escolher várias etapas de processamento, como “Desfocar rostos”, gerar “Resumo por IA” e otimizar o sinal de áudio com “Melhorar a fala”. Você pode até escolher um espaço de trabalho para alocar o recurso — ele, é claro, também estará na seção geral de recursos do Console da Web.
...
- **Captura ao vivo**: ajuste suas configurações de captura ao vivo de acordo com a largura de banda disponível, a sensibilidade de RV do convidado ou se as lentes de proteção da sua câmera estiverem instaladas:
    ...
    - **Otimizar movimento**: Isso diminuirá a taxa de quadros do vídeo, gerando menos carga na largura de banda de upload da sua rede e aumentando sua taxa de bits de streaming. Além disso, essa opção ajuda a reduzir a sensação de enjoo para participantes em RV. Padrão: Desativado
    - **Bloqueio de direção**: Isso “bloqueia” a visão de 360°, independentemente de como você mova a câmera 360°. Se você quiser que o vídeo em 360° acompanhe o movimento da câmera — por exemplo, se quiser “apontar” com a lente frontal para algo —, defina o Bloqueio de direção como Não. Assim, a câmera se comportará como uma câmera tradicional, o que pode ser mais útil para visitas guiadas. Padrão: Sim
    - **Orientação inicial**: Ao definir o Trava de direção como Não, você pode escolher qual lente (frontal ou traseira) deve ser a orientação inicial ao iniciar o vídeo ao vivo. Padrão: Olhando para o operador, pois essa é a maneira mais natural de iniciar uma reunião ao vivo (= lente traseira). A Captura Rápida funciona de maneira diferente (a lente frontal é a orientação inicial por padrão — veja abaixo).
...
- **Captura Rápida**: Ajuste suas configurações de Captura Rápida de acordo com sua taxa de quadros de vídeo preferida, a largura de banda disponível para uploads de vídeos gravados e outras preferências. Recursos relacionados ao mapa, conforme explicado acima (por exemplo, visualização do mapa, anotações no mapa), são possíveis quando um sinal de GPS é recebido e a configuração de localização nas configurações nativas da câmera está ativada (deve estar por padrão). O ícone de localização/GPS no canto superior direito da Captura Rápida deve estar verde. Pode levar alguns instantes para receber o sinal de GPS e estabelecer a conexão.
    - **Resolução**: Aqui você pode alterar a resolução. *(As resoluções 6k são experimentais e exigem uma etapa manual de união de imagens na Galeria antes do envio para o Avatour Web Console.)*
        - **4k** - Esse é o padrão e mantém um bom equilíbrio entre qualidade de vídeo e tamanho do arquivo.
 - **6k a 30 fps** *(requer uma etapa adicional de costura na Galeria)*
        - **6k a 10 fps** *(requer uma etapa adicional de costura na Galeria)* - Isso é útil se você quiser manter o tamanho do arquivo menor do que com os 30 fps, quando a fluidez do movimento é menos importante.
    ...
    - **Bloqueio de direção**: O mesmo que acima, na seção “Captura ao vivo”. A orientação inicial padrão é sempre a lente frontal para a Captura Rápida.
