...
A bateria interna da câmera dura aproximadamente 30 a 45 minutos. Um alerta será exibido quando a bateria estiver ficando fraca. Com uma bateria externa, você pode prolongar o tempo de funcionamento e até mesmo torná-lo infinito, já que é possível trocar as baterias durante o uso.
...
- **Ambientes mais silenciosos / vários participantes próximos à câmera:** 
  Use o alto-falante clipável NoxGear. Ele não tem a mesma alta fidelidade que os alto-falantes para conferências (por exemplo, o Jabra Speak), mas é fácil de prender na camisa e capta as vozes próximas com eficácia.  
...
- **Teste em um único local:** Qualquer ferramenta de teste de velocidade que você use normalmente (por exemplo, [Speedtest](https://www.speedtest.net)) para verificar tanto a largura de banda de upload quanto a de download.   
...
##### Rede móvel
**Opção A: Hotspot e chip SIM fornecidos no kit**  
...
- Solução de problemas:
  ...
  - Verifique se o APN está correto para sua região ([Guia de configuração do APN](https://avatour.com/support/how-do-i-change-the-apn-on-my-glocalme-hotspot)).
...
> As redes móveis podem limitar a largura de banda inesperadamente. Verifique com sua operadora os limites do plano de dados ou entre em contato com o suporte da Avatour se estiver usando nosso hotspot e nosso SIM.
...
Quando vários participantes entram em uma reunião ao vivo do Avatour a partir do mesmo local da câmera 360°, é fundamental gerenciar cuidadosamente o **áudio e a largura de banda**:  
...
Quando vários participantes ingressam em uma reunião ao vivo do Avatour a partir do mesmo local da câmera 360°, o gerenciamento cuidadoso do **áudio e da largura de banda** é fundamental:  
...
- **Modo no Local:** participe da reunião no modo no local quando estiver fisicamente presente próximo à câmera 360°, pois esse modo é otimizado para uso no local:
...
- **Captura Rápida**: Ajuste suas configurações de Captura Rápida de acordo com sua taxa de quadros de vídeo preferida, a largura de banda disponível para uploads de vídeos gravados e outras preferências. Recursos relacionados ao mapa, conforme explicado acima (por exemplo, visualização do mapa, anotações no mapa), estão disponíveis quando um sinal de GPS é recebido e a configuração de localização nas configurações nativas da câmera está ativada (deve estar ativada por padrão). O ícone de localização/GPS no canto superior direito da Captura Rápida deve estar verde. Pode demorar alguns instantes para receber o sinal de GPS e estabelecer a conexão.
  - **Resolução**: Aqui você pode alterar a resolução. *(As resoluções 6k são experimentais e exigem uma etapa manual de união na Galeria antes do upload para o Avatour Web Console.)*
    ...
    - **6k a 10 fps** *(requer uma etapa adicional de costura na Galeria)* - Isso é útil se você quiser manter o tamanho do arquivo menor do que com os 30 fps, quando a suavidade do movimento for menos importante.
